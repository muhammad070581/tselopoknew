...
def log(value, status='OPOK', color='[P1]'):
    value = colors('{color}[{time}] [P1]: {color}{status} [G1]: {color}{value}[CC]'.format(
...
def log_replace(value, status='NTB', color='[G1]'):
...
class inject(object):
    ...
    def start(self):
        try:
            ...
            frontend_domains = open(real_path('/jaka.txt')).readlines()
            frontend_domains = filter_array(frontend_domains)
            if len(frontend_domains) == 0:
                self.log('Frontend Domains not found. Please check jaka.txt', color='[R1]')
                return
...
class domain_fronting(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.log('Bismillah'.format(self.proxy_host, self.proxy_port))
            ...
            self.log('Alhamdulillah'.format(self.proxy_host, self.proxy_port), color='[P1]')
        except OSError:
...
def main():
    print(colors('\n'.join([
       '[G1]Name    : MUHAMMAD AMIN','[CC]'
       '[P1]Script  : Telkomsel Opok & Makstream 2020','[CC]'
       '[R1]Facebook: Jaka Benang','[CC]'
